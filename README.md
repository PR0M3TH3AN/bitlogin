...
- **Protocol-level tests** (`npm test`, 79 tests): Argon2id/ScalarExpand/JCS/
  ...
  6), initial profile/relay-list publication never overwriting an existing
  kind 0/10002/10050 event when the everyday identity is an imported nsec
  (§15.8, §28.1), and registration/password-rotation refusing to overwrite
  another account already registered at the same locator address (§15.6,
  §18.1) — run against an in-memory mock relay
