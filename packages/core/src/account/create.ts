...
export async function registerAccount(
  ...
  const { locatorPrivateKey, capsuleKey: credentialCapsuleKey } =
    await derivePasswordKeys(params.password, normalizedLoginName);
  try {
    const locatorPublicKey = getPublicKeyHex(locatorPrivateKey);
    const checkPool = new RelayPool(params.vaultRelayUrls, {
      authPrivateKey: locatorPrivateKey,
    });
    let existingCapsule;
    try {
      existingCapsule = await readCredentialCapsule(
        checkPool,
        locatorPublicKey,
        credentialCapsuleKey,
        params.timeoutMs,
      );
    } finally {
      checkPool.closeAll();
    ...
    // §15.3 — validate an imported identity before minting any other account secrets.
    const imported = params.everydayPrivateKey !== undefined;
    if (imported && !isValidScalar(params.everydayPrivateKey!)) {
      throw new RegistrationFailedError(
        "The provided key is not a valid secp256k1 private key.",
      );
    }
    ...
    let recoveryCapsuleKey: Uint8Array;
    try {
      ({ recoveryPrivateKey, capsuleKey: recoveryCapsuleKey } =
        deriveRecoveryKeys(bip39Seed));
    } finally {
      wipe(bip39Seed);
    }

    const everydayPrivateKey = imported
    ...
    try {
      ...
      // §15.6 — recovery must be durable before a credential capsule that the
      // caller can use is allowed to exist. A failed recovery quorum therefore
      // exits before credential construction or publication.
      ...
      try {
        recoveryPublish = await publishAndVerify(recoveryPool, recoveryEvent, {
      ...
      if (!recoveryPublish.success) {
        throw new RegistrationFailedError(
          "The recovery capsule did not reach the required relay acknowledgement and readback quorum. No login credential was published. Please retry, or add more vault relays.",
        );
      }

      // §15.5 — password path, embedding the now-durable signed recovery event
      // for keyless rebroadcast repair.
      ...
      if (!credentialPublish.success) {
        throw new RegistrationFailedError(
          "Registration did not reach the required credential acknowledgement and readback quorum. Please retry, or add more vault relays.",
        );
      }
      ...
      wipe(recoveryPrivateKey, recoveryCapsuleKey);
      if (!completed) {
        if (!imported) wipe(everydayPrivateKey);
        wipe(connectionVaultRoot, vaultSudoKey);
      }
    }
  } finally {
    wipe(locatorPrivateKey, credentialCapsuleKey);
  }
}
...
export async function importAccount(
  params: Omit<RegisterAccountParams, "everydayPrivateKey"> & {
    nsecOrHex: string;
  },
): Promise<RegisterAccountResult> {
...
export function decodeEverydayPrivateKey(nsecOrHex: string): Uint8Array {
  ...
  if (trimmed.startsWith("nsec1")) {
    ...
    throw new RegistrationFailedError(
      "Enter a valid nsec (nsec1…) or a 64-character hex private key.",
    );
  }
