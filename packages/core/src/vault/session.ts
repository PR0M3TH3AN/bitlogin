 ...
 *   which the host can only get from the phrase-gated recovery capsule.
 *   enableSudo() wipes the caller's copy of the sudo key on the way in so
 *   exactly one live copy exists at a time.
 */
...
export class VaultSession {
  ...
  enableSudo(vaultSudoKey: Uint8Array): void {
    this.endSudo();
    this.personalPrk = derivePersonalPrk(
      this.connectionVaultRoot,
      vaultSudoKey,
    );
    wipe(vaultSudoKey);
  }
  ...
  private prkForTier(tier: ConnectionTier): Uint8Array {
    if (this.destroyed) {
      throw new Error(
        "This vault session was destroyed; create a new one from the account's root.",
      );
    }
    if (tier === "connectable") return this.vaultPrk;
    if (!this.personalPrk) {
      throw new Error(
        "Personal-tier records require an open sudo window (enableSudo).",
      );
    }
  ...
  async createConnection(
    ...
    const record: ConnectionRecord = {
      ...
      application_binding: params.application_binding ?? {
        origin: null,
        app_pubkey: null,
      },
  ...
  async updateConnection(
    ...
    changes: Partial<
      Pick<
        ConnectionRecord,
        "state" | "label" | "credential" | "application_binding" | "notes"
      >
    >,
    now?: number,
  ): Promise<{ record: ConnectionRecord; event: NostrEvent }> {
  ...
  decryptEvent(event: NostrEvent): Promise<DecryptedConnectionRecord | null> {
    ...
    return decryptConnectionRecordEvent(
      event,
      this.vaultPrk,
      this.personalPrk ?? undefined,
    );
  }
