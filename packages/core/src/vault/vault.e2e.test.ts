...
import { recoverWithPhrase, completeRecoveryWithNewCredentials } from "../account/recover.js";
import { enableConnectionVault } from "../account/enableVault.js";
import { derivePasswordKeys, deriveRecoveryKeys, normalizeLoginName } from "../account/normalize.js";
...
import { publishConnectionRecord, fetchConnectionRecordEvents } from "./sync.js";
...
describe("Connection Vault end-to-end", () => {
  ...
  it("registration mints the roots; records survive a clean-device login; sign-in restores the wallet", async () => {
    ...
    const { record, event } = await sessionA.createConnection({
      ...
      application_binding: { origin: "https://imsatisfied.app", app_pubkey: null }
    });
    ...
    expect(Buffer.from(login.connectionVaultRoot!)).toEqual(Buffer.from(registration.connectionVaultRoot));

    const sessionB = new VaultSession(login.connectionVaultRoot!);
    const listed = await sessionB.listConnections({ relayUrls: vaultRelayUrls, store: new InMemoryKeyValueStore() });
    expect(listed.connections).toHaveLength(1);
    expect(listed.connections[0]!.record.label).toBe("Satisfied spending wallet");
    expect(listed.connections[0]!.record.application_binding.origin).toBe("https://imsatisfied.app");
    expect((listed.connections[0]!.record.credential as { secret: string }).secret).toBe("cd".repeat(32));
  });
  ...
  it("password change carries the roots; phrase recovery restores them on a clean device", async () => {
    ...
    await changePassword({ loginName, oldPassword, newPassword, vaultRelayUrls });
    const login = await loginWithPassword({ loginName, password: newPassword, vaultRelayUrls });
    expect(Buffer.from(login.connectionVaultRoot!)).toEqual(Buffer.from(registration.connectionVaultRoot));
    expect(Buffer.from(login.vaultSudoKey!)).toEqual(Buffer.from(registration.vaultSudoKey));
    ...
    expect(recovered.currentRecoveryPayload.connection_vault_root).toBeDefined();
    ...
    expect(Buffer.from(reborn.connectionVaultRoot!)).toEqual(Buffer.from(registration.connectionVaultRoot));
  });
  ...
  it("a legacy account (no roots) migrates via the phrase ceremony, idempotently", async () => {
    ...
    const operationalPrivB64 = Buffer.from(registration.everydayPrivateKey).toString("base64url");
    ...
    const recoveryPool = new RelayPool(vaultRelayUrls, { authPrivateKey: recoveryKeys.recoveryPrivateKey });
    expect(
      (await publishAndVerify(recoveryPool, strippedRecoveryEvent, { dTag: D_TAG_RECOVERY_CAPSULE })).success
    ).toBe(true);
    ...
    expect(
      (await publishAndVerify(credentialPool, strippedCredentialEvent, { dTag: D_TAG_PASSWORD_CAPSULE })).success
    ).toBe(true);
    credentialPool.closeAll();

    const legacyLogin = await loginWithPassword({ loginName, password, vaultRelayUrls });
    ...
    await expect(
      enableConnectionVault({ loginName, password, phrase: stranger.recoveryPhrase, vaultRelayUrls })
    ...
    const after = await loginWithPassword({ loginName, password, vaultRelayUrls });
    expect(Buffer.from(after.connectionVaultRoot!)).toEqual(Buffer.from(enabled.connectionVaultRoot));
    ...
    expect(Buffer.from(again.connectionVaultRoot)).toEqual(Buffer.from(enabled.connectionVaultRoot));
  });
  ...
  it("a stale replica cannot silently resurrect an old record (§CV9)", async () => {
    ...
    expect(
      (await session.publish({ event: v1, connectionId: record.connection_id, relayUrls: vaultRelayUrls, store }))
        .success
    ).toBe(true);

    // The update reaches only relay 0 — relays 1 and 2 keep the stale v1.
    const current = await session.decryptEvent(v1);
    const { event: v2 } = await session.updateConnection(current!, { label: "wallet v2" });
    expect(
      (
    ...
    const stale = await session.fetchEvents({ relayUrls: [vaultRelayUrls[1]!, vaultRelayUrls[2]!], store });
    ...
    const decrypted = await session.decryptEvent(all.events.get(record.connection_id)!);
