...
export function parseNwcUri(uri: string): NwcCredential {
  ...
  if (!/^nostr\+walletconnect:\/\//iu.test(trimmed)) {
    throw new NwcParseError("Not an NWC URI: it must start with nostr+walletconnect://");
  }
  // URL cannot parse the custom scheme portably; swap it for http only to lex.
  let parsed: URL;
  try {
    parsed = new URL(trimmed.replace(/^nostr\+walletconnect:\/\//iu, "http://"));
  } catch {
    throw new NwcParseError("The NWC URI is malformed.");
  }
  const walletPubkey = parsed.hostname.toLowerCase();
  if (!HEX64.test(walletPubkey)) {
    throw new NwcParseError("The NWC URI's wallet service pubkey is not 64-char hex.");
  }
  ...
  for (const [key, value] of parsed.searchParams.entries()) {
    if (key === "relay") {
      ...
      try {
        ...
        throw new NwcParseError("A relay parameter in the NWC URI is not a valid URL.");
      }
      if (relayUrl.protocol !== "wss:" && relayUrl.protocol !== "ws:") {
        throw new NwcParseError("A relay parameter in the NWC URI is not a websocket URL.");
      }
      relays.push(value);
    } else if (key === "secret") {
      const lowered = value.toLowerCase();
      if (!HEX64.test(lowered)) {
        throw new NwcParseError("The NWC URI's secret is not a 64-char hex client key.");
      }
      secret = lowered;
    } else if (key === "lud16") {
      lud16 = value;
  ...
  if (relays.length === 0) throw new NwcParseError("The NWC URI names no relay.");
  if (secret === null) throw new NwcParseError("The NWC URI carries no secret.");
...
export function validateNwcCredential(credential: unknown): asserts credential is NwcCredential {
  ...
  if (c!.schema !== SCHEMA_CONNECTION_NWC_V1) fail("NWC credential has the wrong schema.");
  ...
  if (!Array.isArray(c!.relays) || c!.relays.length === 0 || c!.relays.some((r) => typeof r !== "string")) {
  ...
  if (c!.lud16 !== null && typeof c!.lud16 !== "string") fail("NWC credential lud16 must be a string or null.");
  if (
    !Array.isArray(c!.extra_params) ||
    c!.extra_params.some((p) => !Array.isArray(p) || p.length !== 2 || typeof p[0] !== "string" || typeof p[1] !== "string")
  ) {
