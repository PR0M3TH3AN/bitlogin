...
import { validateVaultFields, CapsuleValidationError } from "../capsules/validation.js";
import {
  connectionDTag,
...
import { SCHEMA_CONNECTION_V1, SCHEMA_CONNECTION_NWC_V1, type ConnectionRecord } from "./types.js";
...
describe("§CV6 derivation vectors (compatibility contract — never update, only revert)", () => {
  ...
  it("vault PRK", () => {
    expect(bytesToHex(prk)).toBe("51c3b1186d216e29366e28bda2bf254b40a99a1fdb36759363749ba6be15d5de");
  });
  it("vault signing identity", () => {
    expect(deriveVaultPublicKey(prk)).toBe("71084b9ea80ad9c54ccab57f3586e7017c194c2266af188abfb433c6e9498af4");
  });
  ...
  it("personal PRK and record key", () => {
    ...
    expect(bytesToHex(personal)).toBe("bef59847801ae2017f3b45f415c24ab596784392cac23a227261c61e875cf9a3");
...
describe("§CV7 connection identifiers", () => {
  ...
  it("refuses malformed ids and foreign d tags", () => {
    ...
    expect(connectionIdFromDTag("bitlogin:connection:not!base64url!!")).toBeNull();
  });
});
...
describe("§CV5.2 capsule vault fields", () => {
  ...
  it("accepts both-present and both-absent, rejects one-without-the-other", () => {
    ...
    expect(() => validateVaultFields({ connection_vault_root: root, vault_sudo_key: root })).not.toThrow();
    expect(() => validateVaultFields({ connection_vault_root: root })).toThrow(CapsuleValidationError);
    expect(() => validateVaultFields({ vault_sudo_key: root })).toThrow(CapsuleValidationError);
  });
  it("rejects wrong-size and non-base64url values", () => {
    expect(() => validateVaultFields({ connection_vault_root: "AAAA", vault_sudo_key: root })).toThrow();
    expect(() => validateVaultFields({ connection_vault_root: "!!!", vault_sudo_key: root })).toThrow();
  });
});
...
describe("NWC profile (nwc-connections.md §5-§6)", () => {
  ...
  it("refuses missing relay, missing secret, bad pubkey, and non-websocket relays", () => {
    ...
    expect(() => parseNwcUri(`${base}?secret=${"cd".repeat(32)}`)).toThrow(NwcParseError);
    expect(() => parseNwcUri(`${base}?relay=wss%3A%2F%2Fr.example`)).toThrow(NwcParseError);
    expect(() => parseNwcUri(`nostr+walletconnect://nothex?relay=wss%3A%2F%2Fr&secret=${"cd".repeat(32)}`)).toThrow(
      NwcParseError
    );
    expect(() => parseNwcUri(`${base}?relay=https%3A%2F%2Fr.example&secret=${"cd".repeat(32)}`)).toThrow(NwcParseError);
  });
});
...
describe("§CV8 record events", () => {
  ...
  it("encrypts, signs with the vault identity, and round-trips", async () => {
    const event = await buildConnectionRecordEvent({ vaultPrk: prk, recordPrk: prk, record: record(), now: 1_785_400_100 });
  ...
  it("a different vault's key cannot read or even attribute the record", async () => {
    const event = await buildConnectionRecordEvent({ vaultPrk: prk, recordPrk: prk, record: record(), now: 1 });
  ...
  it("personal-tier records are INVISIBLE (null) without the personal prk, readable with it", async () => {
    ...
    const secretRecord = record({ tier: "personal", connection_type: "password" });
    const event = await buildConnectionRecordEvent({ vaultPrk: prk, recordPrk: personalPrk, record: secretRecord, now: 2 });
  ...
  it("rejects a record whose ciphertext id disagrees with the d tag", async () => {
    const event = await buildConnectionRecordEvent({ vaultPrk: prk, recordPrk: prk, record: record(), now: 3 });
    ...
    await expect(
      decryptConnectionRecordEvent({ ...forged, id: event.id, sig: event.sig }, prk)
    ).rejects.toThrow(); // invalid signature for the altered tags
  });

  it("tombstones wipe the credential to its bare schema (§CV11)", () => {
    const dead = tombstoneRecord(record({ notes: "has secrets" }), 1_785_400_500);
...
describe("VaultSession sudo-window policy", () => {
  it("personal-tier work requires an open window, and endSudo closes it", async () => {
    ...
    await expect(
      session.createConnection({
        ...
        credential: { schema: "bitlogin.connection.password.v1", password: "hunter2" }
      })
  ...
  it("updates keep tier and id immutable and stay monotonic within a second", async () => {
    ...
    const updated = await session.updateConnection(current!, { label: "renamed" }, 1_785_400_000);
